...
class ImageViewer(QGraphicsView):
    ...
    # --- MODIFICATION: The signal now just indicates what component items were clicked ---
    ...
    # --- MODIFICATION: Fixes the bug preventing opposing arrows from being drawn ---
    def redraw_connections(self, data_model, show_all, selected_name):
        ...
        # Use a set to track drawn 'inout' pairs to avoid drawing them twice
        drawn_inout_pairs = set()
        ...
        for source_name, details in data_model.components.items():
            for conn_type in ['output', 'inout']:
                for conn in details.get('connections', {}).get(conn_type, []):
                    ...
                    # For 'inout' connections, ensure we only draw them once.
                    if conn_type == 'inout':
                        # frozenset makes the pair order-independent, e.g., (A,B) is same as (B,A)
                        pair = frozenset((source_name, target_name))
                        if pair in drawn_inout_pairs:
                            continue # Already drawn this bidirectional arrow, skip.
                        drawn_inout_pairs.add(pair)
                    
                    # For 'output', we always draw, as A->B is different from B->A.
                    # For 'inout', we draw the first time we encounter the pair.
    ...
    # --- MODIFICATION: Fixes the arrow deletion bug in local view ---
    def mousePressEvent(self, event):
        ...
        if 'idle' in self.current_mode:
            # Check if an arrow was clicked. If so, let the default behavior handle selection
            # and do *not* emit the idle_mode_clicked signal. This prevents MainWindow
            # from clearing the selection, which was the cause of the bug.
            if any(isinstance(item, ArrowItem) for item in items_at_pos):
                super().mousePressEvent(event)
                return

            # If no arrow was clicked, proceed with the normal logic for selecting components or clearing selection.
            component_items_at_pos = [item for item in items_at_pos if isinstance(item, ComponentRectItem)]
